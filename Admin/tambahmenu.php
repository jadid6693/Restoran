<?php
	...
	if(isset($_POST['simpan_btn'])){
		...
		if($_POST['JdlMenu_txt']!="" && $_POST['Jm_rbt']!="" && $_POST['Hrg_txt']!="" ){
			$targetdir="../menu";$targetfile=$targetdir.basename($_FILES["file"]["name"]);$UploadOk=1;
			$JdlMenu=$_POST['JdlMenu_txt'];$IdJenis=$_POST['Jm_rbt'];$HrgMenu=$_POST['Hrg_txt'];$GbrMenu=basename($_FILES['file']['name']);
			if(file_exists($targetfile)){$UploadOk=0;}
			if($UploadOk==1){
				copy($_FILES["file"]["tmp_name"],$targetfile)or die("tdk tercopy");
				//if(move_uploaded_file($_FILES["file"]["tmp_name"],$targetfile)){$hasil="Terupload";}else{$hasil="errorupload";}
			}
	...
	<body>
		...
		<table border="1" style="margin-left:auto;margin-right:auto;width:800px;">
			...
			for($i=0;$i<count($arr_menu);$i++){
				...
				echo "<td><a href='editmenu.php?idmen=".$arr_menu[$i][0]."'>EDIT</a>&nbsp;|&nbsp;<a href='hapusmenu.php?idmen=".$arr_menu[$i][0]."'>HAPUS</a></td>";
		...
		<form method="post" >
			...
			<label>Pilih Gambar : </label><input type="file" name="file" accept=".jpg*" required />
